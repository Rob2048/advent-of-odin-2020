...
main :: proc() {
	...
	for bag_stack_pos < len(bag_stack) {
		// fmt.println("Current", bag_stack[bag_stack_pos])
		current_bag := contains[bag_stack[bag_stack_pos]]

		total_bag_count += 1
		bag_stack_pos += 1

		for bag, value in current_bag {
			// fmt.println(bag, value)
	...
	fmt.println("Valid bag count:", total_bag_count)
}
