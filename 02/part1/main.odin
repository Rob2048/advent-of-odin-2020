...
main :: proc() {
	...
	for  {
		...
		num_min := strconv.atoi(min_str)
		num_max := strconv.atoi(max_str)
		char_count := strings.count(password_str, char_str[0:1])

		if (char_count >= num_min && char_count <= num_max) {
			valid_passwords += 1
		}

		fmt.println(num_min, num_max, char_str[0:1], password_str, char_count)
