...
main :: proc() {
	...
	current_view := make(map[Vec4]bool)
	defer delete(current_view)

	accumulation_view := make(map[Vec4]CellAccum)
	...
	for line in strings.split_lines_iterator(&line_iter) {
		for char, x in line {
			if char == '#' {
				current_view[Vec4{i32(x), i32(line_count), 0, 0}] = true
			}
		}
			...
			for x := src_pos.x - 1; x <= src_pos.x + 1; x += 1 {
				for y := src_pos.y - 1; y <= src_pos.y + 1; y += 1 {
					for z := src_pos.z - 1; z <= src_pos.z + 1; z += 1 {
						for w := src_pos.w - 1; w <= src_pos.w + 1; w += 1 {
							dst_pos := Vec4{x, y, z, w}
...
draw_grid :: proc(grid: map[Vec4]bool) {
	bounds_min := Vec4{bits.I32_MAX, bits.I32_MAX, bits.I32_MAX, bits.I32_MAX}
	bounds_max := Vec4{bits.I32_MIN, bits.I32_MIN, bits.I32_MIN, bits.I32_MIN}

	for cell in grid {
		bounds_min.x = min(bounds_min.x, cell.x)
		bounds_min.y = min(bounds_min.y, cell.y)
		bounds_min.z = min(bounds_min.z, cell.z)
		bounds_min.w = min(bounds_min.w, cell.w)

		bounds_max.x = max(bounds_max.x, cell.x)
		bounds_max.y = max(bounds_max.y, cell.y)
		bounds_max.z = max(bounds_max.z, cell.z)
		bounds_max.w = max(bounds_max.w, cell.w)
	}

	for w := bounds_min.w; w <= bounds_max.w; w += 1 {
